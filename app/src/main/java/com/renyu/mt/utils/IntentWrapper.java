    ...
    public static List<IntentWrapper> getIntentWrapperList() {
        if (sIntentWrapperList == null) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
                ...
                boolean ignoringBatteryOptimizations = pm.isIgnoringBatteryOptimizations(Utils.getApp().getPackageName());
                if (!ignoringBatteryOptimizations) {
                    Intent dozeIntent = new Intent(Settings.ACTION_REQUEST_IGNORE_BATTERY_OPTIMIZATIONS);
                    dozeIntent.setData(Uri.parse("package:" + Utils.getApp().getPackageName()));
            ...
            xiaomiGodIntent.putExtra("package_name", Utils.getApp().getPackageName());
            ...
            //三星 5.0/5.1 自启动应用程序管理
            Intent samsungLIntent = Utils.getApp().getPackageManager().getLaunchIntentForPackage("com.samsung.android.sm");
            ...
            meizuIntent.putExtra("packageName", Utils.getApp().getPackageName());
    ...
    public static String getApplicationName() {
        if (sApplicationName == null) {
            ...
            try {
                ...
                ai = pm.getApplicationInfo(Utils.getApp().getPackageName(), 0);
                sApplicationName = pm.getApplicationLabel(ai).toString();
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
                sApplicationName = Utils.getApp().getPackageName();
            }
        }
